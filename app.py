...
def _make_news_config() -> NewsConfig:
    """Build a newspaper3k Config that mimics a real Chrome browser."""
    cfg = NewsConfig()
    ...
    cfg.language      = "en"          # parse English text only
    cfg.request_timeout = 10
    cfg.fetch_images  = False          # skip images — faster
    cfg.memoize_articles = False
    return cfg


def _parse_article_title(url: str, cfg: NewsConfig) -> str | None:
    """Try to download & parse a single article title via newspaper3k."""
    try:
...
def fetch_aljazeera(keyword: str) -> list[str]:
    """
    3-tier strategy to fetch Al Jazeera headlines in English.

    Tier 1 — newspaper3k  : parse individual article pages linked from search.
    Tier 2 — BeautifulSoup: scrape h2/h3 tags from the search result HTML.
    Tier 3 — RSS fallback : pull from Al Jazeera's public English RSS feed.
    """
    AJ_SEARCH = (
        f"https://www.aljazeera.com/search/{requests.utils.quote(keyword)}"
    )
    cfg = _make_news_config()
    titles: list[str] = []

    # ── Tier 1: newspaper3k on linked articles ──────────────────────────────
    try:
        r = requests.get(AJ_SEARCH, headers=HEADERS, timeout=10)
        r.raise_for_status()
        soup = BeautifulSoup(r.text, "html.parser")
        ...
        for a_tag in soup.find_all("a", href=True):
            href: str = a_tag["href"]
            # Al Jazeera article URLs follow /YYYY/MM/DD/ or /news/ patterns
            if any(seg in href for seg in ["/news/", "/economy/", "/sports/",
                                           "/science/", "/features/", "/opinion/"]):
                full = (
                    href if href.startswith("http")
                    else f"https://www.aljazeera.com{href}"
                )
                if full not in article_links:
                    article_links.append(full)
            if len(article_links) == 8:   # fetch a few extras in case some fail
                break

        for link in article_links:
            title = _parse_article_title(link, cfg)
    ...
    # ── Tier 2: BeautifulSoup direct scrape of the search page ─────────────
    try:
        r = requests.get(AJ_SEARCH, headers=HEADERS, timeout=10)
        r.raise_for_status()
        soup = BeautifulSoup(r.text, "html.parser")
        for tag in soup.find_all(["h2", "h3"], limit=60):
            text = tag.get_text(strip=True)
            if len(text) > 20 and text not in titles:
                titles.append(text)
            if len(titles) == 5:
                break
    except Exception:
    ...
    try:
        ...
        for item in soup.find_all("item"):
            ...
            if len(titles) == 5:
                break
        # If keyword filter yields nothing, take the latest 5
        if not titles:
